...
Module CardPredict
    ...
    Sub Main()
        ...
        Do
            ...
            Select Case Choice
                ...
                Case "6"
                    Read_Scores_From_file(RecentScores)
                Case "7"
                    Sort_recent_scores(RecentScores)
            End Select
        Loop Until Choice = "q"
    End Sub
    ...
    Sub DisplayMenu()
        ...
        Console.WriteLine("5.  Save recent scores to file")
        Console.WriteLine("6.  Load recent scores from file")
    ...
    Sub UpdateRecentScores(ByRef RecentScores() As TRecentScore, ByVal Score As Integer)
        Dim PlayerName As String
        Dim Count As Integer
        Dim FoundSpace, matchname As Boolean
        ...
        End If
        If FoundSpace = True Or matchname = True Then
    ...
    Sub SaveRecentScores(ByVal recentscores() As TRecentScore)
        ...
        filename = "recent_scores"

        FileOpen(1, filename, OpenMode.Output)
        For count = 1 To NoOfRecentScores
            WriteLine(1, recentscores(count).Name, recentscores(count).Score)
        Next

        FileClose(1)
    End Sub

    Sub Read_Scores_From_file(ByRef recentscores() As TRecentScore)
        Dim filename As String
        Dim playername As String = ""
    ...
    Sub Sort_recent_scores(ByVal recentscores() As TRecentScore)
        Dim temp As TRecentScore
        If recentscores(2).score > recentscores(1).score Then
            temp = recentscores(1)
            recentscores(1) = recentscores(2)
            recentscores(2) = temp
        End If

        If recentscores(3).Score > recentscores(2).Score Then
            temp = recentscores(2)
            recentscores(2) = recentscores(3)
            recentscores(3) = temp
        End If
